...
const EigenvectorElem   = ComplexF64
const Eigenvector       = Vector{EigenvectorElem}
const EigenvectorMatrix = Matrix{EigenvectorElem}

"""
    Fockstates

Struct for Fock states, generated by specifying the number of orbitals `norb`, e.g. `fs = Fockstates(;norb = 4)`.

Fields
-------------
- **`NStates`** : Integer, specifying the number of states, i.e. `4^norb`
- **`norb`**    : Integer, specifying the number of orbitals.
- **`states`**  : An array of arrays for all Fock states for Quantum numbers N,S: `typeof(allstates[N][S][i])=Fockstate`, `states[n][s][i]`  (`n`: electron number + 1, `s`: spin index, `i` state in NS block)
"""
struct Fockstates
    Nstates::Int64
    norb::Int64
